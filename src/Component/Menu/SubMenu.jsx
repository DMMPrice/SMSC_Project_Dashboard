...
const SubMenu = () => {
    ...
    const subItems = menu?.submenu?.filter((sub) => sub.allowedRoles.includes(userRole)) || [];

    return (<div className="min-h-screen p-6 flex flex-col items-center">
        <button
        ...
        {!subItems.length ? (<div className="text-center text-red-500 text-lg font-medium mt-12">
            You do not have permission to view this section.
        </div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-5xl w-full">
            {subItems.map((item) => (<div
                key={item.title}
